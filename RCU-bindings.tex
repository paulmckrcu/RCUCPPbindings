...
\co{-luruc-signal} to select signal-based RCU.
...
\begin{verbatim}
 ...
 7     }
 8
 9     explicit rcu_scoped_reader(class rcu_domain *rd) noexcept
10     {
11       this->rd = rd;
12       rd->read_lock();
13     }
...
These methods take a deleter or not and take an
\co{rcu_domain} class or not.
If supplied, the function is invoked after a grace period,
otherwise the object is handed to \co{delete}, courtesy of the defaults
for the \co{d} parameters to the \co{call()} member functions.
...
(Use of lambda capture requires either memory allocation on the free path,
which is not good practice, or requires that only one type of lambda be
used for a given RCU-protected class, which is rather inconvenient.)
